...
#define rotphi(i,j,k) (rotphi[ijk(i,j,k)])
...
printf("Setting up rotational velocity...\n");
fflush(stdout);
...
void get_U(Float mu)	// Find U
{
	...
	for (k = 0; k <= Nz; k++)
		for (j = 0; j <= Ny; j++)
			for (i = 0; i <= Nx; i++)
		U(i, j, k) = c * density(i, j, k) + phiU(i, j, k) + rotphi(i,j,k) - mu;
}
...
//*********************************************************************
// Effective rotational potential
//*********************************************************************
void get_Rot()	
{
	int i, j, k;
